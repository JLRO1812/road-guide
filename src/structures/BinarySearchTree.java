...
public class BinarySearchTree <K extends Comparable<K>, E> implements BinarySearchTreeI<K, E>{
	...
	public BinarySearchTree(INode<K,E> r) {
		root = r;
		size = 1;
	}
	...
	protected void insert(INode<K,E> n) {
		if(root==null) {
			root = n;
		}else {
			INode<K,E> aux = root;
	...
	public E search(K key) {
		if(root!=null) {
			...
			INode<K, E> aux = (key.compareTo(root.getKey())<0)?root.getLeft():root.getRight();
			while(aux!=null) {
	...
	private INode<K,E> searchNode(K key) {
		if(root!=null) {
			if(root.getKey()==key)
				return root;
	...
	@Override
	public INode<K,E> delete(K key) {
		INode<K,E> nodeToDelete = searchNode(key);
		...
		INode<K,E> aux = nodeToDelete.getFather();
		...
		if(nodeToDelete.childrensAmount() == 0) {
			...
			INode<K,E> aux2 = (nodeToDelete.getLeft()!=null)?nodeToDelete.getLeft():nodeToDelete.getRight(); 
			aux2.setFather(aux);
			if(aux==null)
				root = aux2;
			else if(nodeToDelete.getKey().compareTo(aux.getKey())>0)
			...
			INode<K,E> delete = delete(s);
			size++;
			INode<K,E> replace = new Node<K,E>(delete.getKey(), delete.getValue(),
	...
	private K getSussesor(INode<K,E> aux){
		if(aux==null)
			return null;
	...
	private String inOrderK(String n, INode<K,E> aux) {
		if(aux==null)
			return n;
		else {
	...
	private String inOrderV(String n, INode<K,E> aux) {
		if(aux==null)
			return n;
		else {
	...
	private String preOrderK(String n, INode<K,E> aux) {
		if (aux != null) { 
            n += (aux.getKey() + " "); 
	...
	private String preOrderV(String n, INode<K,E> aux) {
		if (aux != null) { 
            n += (aux.getValue() + " "); 
	...
	public INode<K,E> getLess(INode<K,E> n) {
		INode<K,E> aux = n;
		if(aux==null)
			return null;
	...
	public INode<K,E> getHigher(INode<K,E> n) {
		INode<K,E> aux = n;
		if(aux==null)
			return null;
